    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-10">
      ...
      <div className="text-center max-w-3xl mx-auto space-y-4">
        ...
        <h1 className="text-3xl sm:text-5xl font-extrabold text-slate-900 tracking-tight leading-tight">
          Side-by-Side <span className="bg-gradient-to-r from-violet-600 via-fuchsia-600 to-emerald-600 bg-clip-text text-transparent">Company Radar Comparison</span>
        </h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {PRESET_COMPARISONS.map((preset) => (
            <button
              ...
              className="p-3.5 rounded-xl bg-slate-50 hover:bg-violet-50/60 border border-slate-200 hover:border-violet-300 text-left transition group space-y-1 shadow-sm"
            >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="px-3 py-1.5 bg-white border border-slate-300 rounded-lg text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-violet-500 shadow-sm"
            />
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          {selectedTrends.map((trend) => (
            <div
              ...
              className="flex items-center gap-2.5 px-3.5 py-2 rounded-xl bg-violet-50 border border-violet-200 text-violet-900 shadow-sm"
            >
              <div className="w-6 h-6 rounded-lg bg-violet-600 flex items-center justify-center text-xs font-bold text-white">
              ...
              <button
                ...
                className="text-slate-400 hover:text-rose-600 disabled:opacity-30 transition p-0.5"
              >
        ...
        <div className="glass-card rounded-2xl p-5 sm:p-6 border border-slate-200 bg-white shadow-sm flex flex-col justify-between space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-500 hover:to-purple-500 text-white font-bold text-xs transition shadow-md shadow-violet-500/20 active:scale-98"
            >
      ...
      <div className="space-y-4">
        ...
        <div className={`grid grid-cols-1 ${selectedTrends.length === 3 ? 'lg:grid-cols-3' : 'lg:grid-cols-2'} gap-6`}>
          {selectedTrends.map((trend) => (
            <div
              key={trend.company.id}
              className="glass-card rounded-2xl p-5 sm:p-6 border border-slate-200 bg-white shadow-sm space-y-4 flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between pb-3 border-b border-slate-100 mb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center font-bold text-slate-800 text-xs">
                      {trend.company.name.charAt(0)}
                    </div>
                    <div>
                  ...
                  <Link
                    ...
                    className="text-xs text-violet-600 hover:text-violet-800 font-semibold flex items-center gap-0.5"
                  >
              ...
              <div className="pt-3 border-t border-slate-100 text-[11px]">
                <span className="text-slate-500">Company Unique Focus: </span>
        ...
        <div className={`grid grid-cols-1 ${selectedTrends.length === 3 ? 'lg:grid-cols-3' : 'lg:grid-cols-2'} gap-6`}>
          {selectedTrends.map((trend) => (
            <div
              ...
              className="glass-card rounded-2xl p-5 sm:p-6 border border-slate-200 bg-white shadow-sm space-y-3"
            >
              <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                <span className="text-xs font-bold text-slate-900">{trend.company.name} Rounds</span>
                <span className="text-[11px] text-slate-500">{trend.totalRounds} rounds classified</span>
              </div>
