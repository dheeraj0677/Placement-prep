  ...
  return (
    <div className="min-h-screen py-8 sm:py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-10">
      ...
      <div className="flex items-center gap-2 text-xs text-slate-400">
        ...
        <span>/</span>
        <Link href="/career-paths" className="hover:text-white transition">Career Compass</Link>
        <span>/</span>
        <span className="text-violet-400 font-semibold truncate">{role.title}</span>
      </div>
      ...
      <div className="glass-card rounded-3xl p-6 sm:p-10 border border-slate-800 relative overflow-hidden shadow-2xl">
        ...
        <div 
          className={`absolute top-0 right-0 w-96 h-96 rounded-full blur-3xl pointer-events-none -z-10 ${
            isTech ? 'bg-emerald-600/10' : 'bg-violet-600/15'
          }`} 
        />
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-4 max-w-3xl">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {role.badge && (
                <span className="px-2.5 py-1 text-xs font-semibold rounded-md bg-amber-500/10 text-amber-300 border border-amber-500/20 flex items-center gap-1">
              ...
              <span className="px-2.5 py-1 text-xs font-semibold rounded-md bg-slate-800 text-slate-300 border border-slate-700">
                Difficulty: <span className="font-bold text-white">{role.difficultyLevel}</span>
              </span>
            </div>
            ...
            <div className="flex items-start gap-4">
              <div
                className={`w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center shrink-0 border shadow-lg ${
                  isTech
                    ? 'bg-emerald-600/20 text-emerald-400 border-emerald-500/40 shadow-emerald-500/15'
                    : 'bg-violet-600/20 text-violet-400 border-violet-500/40 shadow-violet-500/15'
                }`}
              >
          ...
          {/* Compensation Card */}
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shrink-0 lg:min-w-[260px] text-center lg:text-right space-y-1">
            ...
            <div className="text-xs text-slate-400">
              National Average: <span className="text-white font-bold">{role.averageCTC}</span>
            </div>
          </div>
        </div>
      </div>
            ...
            <ul className="space-y-3">
              {role.responsibilities.map((resp, idx) => (
                <li key={idx} className="flex items-start gap-3 text-xs sm:text-sm text-slate-300">
                  <div className="w-5 h-5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 flex items-center justify-center shrink-0 mt-0.5 font-bold text-[10px]">
                    {idx + 1}
                  </div>
          ...
          <div className="glass-card rounded-2xl p-6 sm:p-7 border border-slate-800 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-4 rounded-xl bg-slate-900/80 border border-slate-800 space-y-1">
                <div className="text-xs text-slate-400 font-semibold uppercase">Eligible Degrees</div>
              ...
              <div className="p-4 rounded-xl bg-slate-900/80 border border-slate-800 space-y-1">
                <div className="text-xs text-slate-400 font-semibold uppercase">Min. CGPA Cutoff</div>
                <div className="text-xs sm:text-sm font-bold text-white">
            ...
            <div className="p-4 rounded-xl bg-slate-900/80 border border-slate-800 space-y-2">
              <div className="text-xs text-slate-400 font-semibold uppercase">Branch Criteria</div>
            ...
            <div className="space-y-2 pt-1">
              <div className="text-xs text-slate-400 font-semibold uppercase">Key Prerequisites:</div>
          ...
          <div className="glass-card rounded-2xl p-6 sm:p-7 border border-slate-800 space-y-4">
            ...
            <div className="space-y-3">
              {role.interviewRounds.map((round, idx) => (
                <div
                  key={idx}
                  className="p-4 rounded-xl bg-slate-900/80 border border-slate-800 space-y-1.5 hover:border-violet-500/40 transition"
                >
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="px-2 py-0.5 text-[11px] font-bold rounded bg-violet-500/15 text-violet-300 border border-violet-500/30">
                        {stage.phase}
                      </span>
                      <span className="text-xs text-slate-400 font-mono">({stage.timeline})</span>
                    </div>
                    <span className="text-xs font-semibold text-white">{stage.focus}</span>
                  </div>
          ...
          <div className="glass-card rounded-2xl p-6 sm:p-7 border border-slate-800 space-y-4">
            ...
            <div className="space-y-3">
              {role.careerLadder.map((step, idx) => (
                <div
                  key={idx}
                  className="flex items-start gap-3 p-3.5 rounded-xl bg-slate-900/70 border border-slate-800"
                >
                  <div className="px-2 py-1 rounded bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 font-mono text-[11px] font-bold shrink-0">
                    {step.years}
                  </div>
          ...
          <div className="glass-card rounded-2xl p-5 sm:p-6 border border-slate-800 space-y-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {role.softSkills.map((skill) => (
                <span
                  key={skill}
                  className="px-2.5 py-1 text-xs rounded-lg bg-slate-900 text-slate-300 border border-slate-800 font-medium"
                >
                  {skill}
                </span>
              ))}
          ...
          <div className="glass-card rounded-2xl p-5 sm:p-6 border border-slate-800 space-y-4">
            ...
            <div className="space-y-2.5">
              {role.hiringCompanies.map((comp) => (
                <div
                  key={comp.name}
                  className="p-3 rounded-xl bg-slate-900/80 border border-slate-800 flex items-center justify-between gap-3 hover:border-violet-500/40 transition group"
                >
                  <div>
                    <div className="text-xs sm:text-sm font-bold text-white group-hover:text-violet-300 transition">
                      {comp.name}
                    </div>
                    <div className="text-[10px] text-slate-400">{comp.hiringType}</div>
                  </div>
                  ...
                  <div className="text-right">
                    ...
                    {comp.companyId ? (
                      <Link
                        ...
                        className="text-[10px] text-violet-400 hover:underline flex items-center gap-0.5 justify-end"
                      >
              ...
              <div className="space-y-2">
                {role.recommendedGuideSlugs.map((slug) => (
                  <Link
                    key={slug}
                    href={`/guides/${slug}`}
                    className="flex items-center justify-between p-3 rounded-xl bg-slate-900/80 hover:bg-slate-850 border border-slate-800 hover:border-violet-500/40 text-xs font-semibold text-slate-200 hover:text-white transition group"
                  >
                    <span className="capitalize">{slug.replace(/-/g, ' ')} Master Guide</span>
                    <ArrowRight className="w-3.5 h-3.5 text-violet-400 group-hover:translate-x-0.5 transition" />
                  </Link>
                ))}
              </div>
            </div>
