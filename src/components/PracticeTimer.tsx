...
export default function PracticeTimer() {
        ...
        <button
          ...
          className={`group flex items-center gap-2.5 px-4 py-2.5 rounded-full backdrop-blur-xl border shadow-xl transition duration-300 active:scale-95 ${
            isRunning
              ...
              : 'bg-white hover:bg-slate-50 text-slate-700 hover:text-slate-900 border-slate-200 shadow-md'
          }`}
        >
          <Clock className={`w-4 h-4 ${isRunning ? 'animate-spin' : 'text-violet-600'}`} style={{ animationDuration: '4s' }} />
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 transition-all duration-300">
      <div className="glass-card rounded-2xl p-5 border border-slate-200 shadow-2xl w-80 space-y-4 relative bg-white/95 backdrop-blur-2xl">
        {/* Header */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-slate-900">DSA Practice Timer</span>
          </div>
        ...
        <div className="grid grid-cols-2 p-1 bg-slate-100 rounded-xl border border-slate-200 text-xs font-semibold">
          <button
            ...
            className={`py-1.5 rounded-lg transition ${
              mode === 'countdown' ? 'bg-gradient-to-r from-violet-600 to-purple-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
            Countdown
          </button>
          <button
            ...
            className={`py-1.5 rounded-lg transition ${
              mode === 'stopwatch' ? 'bg-gradient-to-r from-violet-600 to-purple-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
            Stopwatch
          </button>
        ...
        {/* Big Time Display */}
        <div className="text-center py-3 bg-slate-50 rounded-xl border border-slate-200 relative overflow-hidden">
          {/* Progress bar background in countdown mode */}
          {mode === 'countdown' && (
            <div
              className="absolute left-0 bottom-0 top-0 bg-violet-500/10 transition-all duration-1000 -z-0 pointer-events-none"
          ...
          <div className="text-[11px] text-slate-500 mt-1 flex items-center justify-center gap-1 font-medium">
            {isFinished ? (
              ...
              <span className="text-emerald-700 flex items-center gap-1">
                <Flame className="w-3 h-3 animate-pulse text-emerald-600" /> Focus Mode Active
              </span>
            ) : (
