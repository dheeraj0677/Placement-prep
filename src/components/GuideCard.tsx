...
export default function GuideCard({ guide }: GuideCardProps) {
  ...
  return (
    <div className="glass-card-hover rounded-2xl p-5 sm:p-6 flex flex-col justify-between h-full group border border-slate-200 relative overflow-hidden transition-all duration-300">
      ...
      <div className="absolute top-0 right-0 w-36 h-36 bg-violet-500/5 group-hover:bg-violet-500/10 rounded-full blur-2xl pointer-events-none transition duration-500" />
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-start gap-3.5 pt-1">
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 border shadow-md ${tagStyle.bg} ${tagStyle.border} ${tagStyle.text} group-hover:scale-105 transition duration-300`}>
          ...
          <div className="space-y-1">
            ...
            <span className="inline-flex items-center gap-1 text-[11px] text-violet-700 font-semibold">
        ...
        {/* Stats Row */}
        <div className="grid grid-cols-2 gap-2 py-2 px-3 rounded-xl bg-slate-50 border border-slate-200 text-xs">
          <div className="flex items-center gap-2 text-slate-600">
            <Target className="w-3.5 h-3.5 text-emerald-600" />
            <span><strong className="text-slate-900 font-bold">{guide.problems.length}</strong> Questions</span>
          </div>
          <div className="flex items-center gap-2 text-slate-600">
            <Clock className="w-3.5 h-3.5 text-indigo-600" />
            <span><strong className="text-slate-900 font-bold">~{guide.estimatedHours}h</strong> Roadmap</span>
          </div>
        </div>
      ...
      <div className="pt-5 mt-4 border-t border-slate-200 flex items-center justify-between">
        ...
        <Link
          ...
          className="inline-flex items-center gap-1.5 text-xs font-bold text-violet-600 group-hover:text-violet-700 transition"
        >
